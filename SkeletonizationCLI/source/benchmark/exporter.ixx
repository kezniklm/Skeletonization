...
export module benchmark:exporter;
...
{
	export struct exporter_options
	{
		// Output formatting
		bool pretty = true;
			...
			try
			{
					     ...
					     note] : packages)
				{
					...
					for (std::size_t index = 0; index < container.size(); ++index)
					{
						...
						const std::string id = make_image_id(container.name(), label, index);

						set_string_member(image, "id", id, allocator);
						set_string_member(image,
						                  "label",
				...
				{
					std::ofstream ofstream(out_json_path, std::ios::binary | std::ios::trunc);
	...
	private:
		static void set_string_member(rapidjson::Value& obj, const char* key,
		                              const std::string& s,
		...
		static void add_if_nonempty_string(rapidjson::Value& obj, const char* key,
		                                   const std::string& s,
		                                   rapidjson::Document::AllocatorType& alloc)
		{
			if (!s.empty())
		...
		template <class T>
		static void add_if_positive(rapidjson::Value& obj, const char* key,
		                            const T& v,
		                            rapidjson::Document::AllocatorType& alloc)
		{
		...
		static void add_timestamp_member(rapidjson::Document& document,
		                                 rapidjson::Document::AllocatorType& allocator)
		{
			...
			value.SetString(timestamp.data(), static_cast<rapidjson::SizeType>(timestamp.size()), allocator);

			document.AddMember(key, value, allocator);
		}

		static std::string make_timestamp_iso8601_local()
		{
		...
		static bool encode_mat_to_base64(const cv::Mat& matrix, const exporter_options& opts, std::string& out_b64)
		{
			...
			else if (equal_icase(opts.image_ext, ".jpg") || equal_icase(opts.image_ext, ".jpeg"))
			{
		...
		static bool base64_encode(const std::vector<uchar>& in, std::string& out)
		{
		...
		static std::string make_image_id(const std::string_view container_name,
		...
		static std::string sanitize_id_component(const std::string_view s)
		{
			std::string output;
		...
		static bool equal_icase(const std::string_view a, const std::string_view b)
		{
			if (a.size() != b.size())
			{
				return false;
			...
			{
				if (std::tolower(static_cast<unsigned char>(a[index])) != std::tolower(
					static_cast<unsigned char>(b[index])))
				{
					return false;
				}
			}
