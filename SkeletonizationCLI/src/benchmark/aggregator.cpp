...
namespace skeletonization_benchmark
{
	namespace
	{
			...
			original_images_map& original_images_map)
		{
			for (const auto& runner : runners | std::views::values)
			{
				...
				for (std::size_t i = skip_original_index; i < image_container.size(); ++i)
				{
					const auto full_key = image_container.label(i);

					const auto display = [&]()
					{
						const auto p = full_key.find('/');
						return p == std::string::npos ? full_key : full_key.substr(p + 1);
					}();
					...
					image_algorithm_record_map[image_name].push_back(
						algorithm_record{display, std::move(image), image_metrics});
				}
			}
		}

		summary compute_summary(
			const std::vector<visual_inspector::image_metrics>& image_metrics_list)
		{
			...
			for (std::size_t i = 1; i < image_metrics_list.size(); ++i)
			{
			...
			const double avg_ms = total_ms /
				static_cast<double>(total_number);
			const int64_t avg_it = total_iterations /
				static_cast<int64_t>(total_number);
			...
			const double stddev_ms =
				total_number > 1 ? std::sqrt(sum_sq / total_number) : 0.0;
			const double thr = avg_ms > 0.0 ? 1000.0 / avg_ms : 0.0;

			return summary{
				.avg_ms = avg_ms,
				.min_ms = (min_ms == std::numeric_limits<double>::max()
					           ? 0.0
					           : min_ms),
				.max_ms = max_ms,
		...
		aggregator::package make_package(
			...
			const original_images_map& originals)
		{
			...
			if (const auto it = originals.find(image_name); it != originals.end())
			{
				...
				image_metrics_list.emplace_back();
			}

			std::ranges::sort(
				algorithm_records,
				[](const algorithm_record& lhs, const algorithm_record& rhs)
				{
	...
	std::vector<aggregator::package> aggregator::build(
		const std::vector<std::pair<std::string, std::unique_ptr<runner>>>& runners,
		const metrics_map& metrics_map)
	{
		...
		{
			packages.push_back(
				make_package(image_name, algorithm_records, originals));
		}
