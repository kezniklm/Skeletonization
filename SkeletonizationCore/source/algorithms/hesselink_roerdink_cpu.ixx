...
{
	export class hesselink_roerdink_cpu final : public skeletonizer_cpu,
	...
	private:
		...
		static cv::Mat compute_horizontal_feature_map(const cv::Mat& binary_image)
		{
			...
			for (auto row = 0; row < binary_image.rows; ++row)
			{
				...
				distance_map_row[binary_image.cols - 1] = binary_image_row[binary_image.cols - 1] == pixel_background
					                                          ? 0
					                                          : max_distance;

				for (auto column = binary_image.cols - 2; column >= 0; --column)
				{
		...
		static cv::Mat compute_vertical_feature_map(const cv::Mat& binary_image, const cv::Mat& horizontal_feature_map)
		{
			...
			for (int column = 0; column < binary_image.cols; ++column)
			{
				...
				auto last_candidate_index = 0;
				candidate_positions[0] = 0;
				boundary_positions[0] = 0;

				for (auto current_row = 1; current_row < binary_image.rows; ++current_row)
				{
					...
					{
						const auto numerator = (current_row + candidate_positions[last_candidate_index]) *
							(current_row - candidate_positions[last_candidate_index]) +
							squared_distances_to_boundary[current_row] -
							squared_distances_to_boundary[candidate_positions[last_candidate_index]];

						const auto denominator = 2 * (current_row - candidate_positions[last_candidate_index]);
						const auto new_boundary = 1 + numerator / denominator;
				...
				for (int current_row = binary_image.rows - 1; current_row >= 0; --current_row)
				{
					...
					vertical_feature_map.at<int>(current_row, column) = feature_values_in_column[chosen_candidate] *
						binary_image.rows + chosen_candidate;
		                                              ...
		                                              cv::Mat& output)
		{
			for (int column = 0; column < binary_image.cols; ++column)
			{
				for (int row = 1; row < binary_image.rows; ++row)
				{
					if (column > 0 && (!is_pixel_background(binary_image, column, row) || !is_pixel_background(
					...
					if (row > 0 && (!is_pixel_background(binary_image, column, row) || !is_pixel_background(
