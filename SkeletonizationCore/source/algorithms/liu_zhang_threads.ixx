...
export module skeletonizer_cpu:liu_zhang_threads;

import :threads_core;

export namespace skeletonizer::cpu::algorithms
{
	class liu_zhang_threads final : public skeletonizer_threads, public ::skeletonizer::algorithms::liu_zhang
	{
	public:
			...
			cv::parallel_for_(cv::Range(1, binary_image.rows - 1), [&](const cv::Range& range)
			{
				for (auto row = range.start; row < range.end; ++row)
				{
					...
					for (auto column = 1; column < binary_image.cols - 1; ++column)
					{
						...
						if (a == 1 && b >= 2 && b <= 6 && step_condition_c == 0 && step_condition_d ==
							0 && is_cp_ok)
						{
		...
		void delete_patterns_ghij(cv::Mat& binary_image, cv::Mat& marker) const
		{
			...
			cv::parallel_for_(cv::Range(1, binary_image.rows - 1), [&](const cv::Range& range)
			{
				for (auto row = range.start; row < range.end; ++row)
				{
					...
					const auto current = binary_image.ptr<uchar>(row);
					const auto next = binary_image.ptr<uchar>(row + 1);
					const auto marker_row = marker.ptr<uchar>(row);
					...
					for (auto column = 1; column < binary_image.cols - 1; ++column)
					{
						...
						const auto p2 = previous[column] > 0;
						const auto p3 = previous[column + 1] > 0;
						const auto p4 = current[column + 1] > 0;
						const auto p5 = next[column + 1] > 0;
						const auto p6 = next[column] > 0;
						const auto p7 = next[column - 1] > 0;
						const auto p8 = current[column - 1] > 0;
						const auto p9 = previous[column - 1] > 0;
