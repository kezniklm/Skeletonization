...
export module configuration:creators_common;
...
export namespace configuration
{
	...
	struct algorithm_entry
	{
		...
		static constexpr bool has_cpu = !std::is_same_v<CpuImplementation, std::nullptr_t>;
		static constexpr bool has_thread = !std::is_same_v<ThreadedImplementation, std::nullptr_t>;
#if SKELETONIZATION_WITH_GPU
		static constexpr bool has_gpu = !std::is_same_v<GpuImplementation, std::nullptr_t>;
#else
		static constexpr bool has_gpu = false;
#endif

		static_assert(has_cpu || has_thread || has_gpu,
		              "AlgorithmEntry must have at least one implementation type (cpu/thread/gpu)");
		...
		static void push_creators_for_type(skeletonizer::skeletonizer_type type, std::vector<skeletonizer_creator>& out)
		{
	...
	template <class CpuT = std::nullptr_t, class ThreadT = std::nullptr_t, class GpuT = std::nullptr_t>
