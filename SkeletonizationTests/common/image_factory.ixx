...
export module tests_common:image_factory;

namespace skeltest
{
	[[nodiscard]] static cv::Mat mk(const int width, const int height)
	{
		return cv::Mat(height, width, CV_8U, cv::Scalar(0));
	}

	[[nodiscard]] static cv::Mat generate_plus_sign()
	{
	...
	[[nodiscard]] static cv::Mat generate_hollow_rectangle_with_tail()
	{
	...
	[[nodiscard]] static cv::Mat generate_two_touching_blobs()
	{
	...
	[[nodiscard]] static cv::Mat generate_zig_zag()
	{
		auto image = mk(200, 60);

		constexpr int thickness = 3;

		const std::vector<cv::Point> pts{{5, 50}, {40, 10}, {80, 50}, {120, 10}, {160, 50}, {195, 10}};
	...
	[[nodiscard]] static cv::Mat generate_blob_with_hole()
	{
		auto image = mk(128, 128);
	...
	[[nodiscard]] static cv::Mat generate_small_diagonal()
	{
		auto image = mk(9, 9);
	...
	[[nodiscard]] static cv::Mat generate_single_pixel()
	{
		auto image = mk(17, 17);
	...
	[[nodiscard]] static cv::Mat generate_empty_image()
	{
		return mk(64, 64);
	}

	[[nodiscard]] static cv::Mat generate_full_image()
	{
		auto image = mk(64, 64);
	...
	[[nodiscard]] static cv::Mat gen_random_strokes()
	{
		auto image = mk(180, 180);

		std::mt19937 rng(42);

		std::uniform_int_distribution x(5, 175), y(5, 175);
	...
	static const std::array<generator_function, 10>& generators()
	{
	...
	[[nodiscard]] static std::vector<cv::Mat> make_images()
	{
	...
	export const std::vector<cv::Mat>& test_images()
	{
