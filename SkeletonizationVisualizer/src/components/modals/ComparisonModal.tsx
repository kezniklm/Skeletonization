      ...
      >
        ...
        >
          ...
          <p id={descId} className="sr-only">
            Compare original and processed images using side-by-side, slider, or overlay modes.
          </p>
        ...
        <div className={`flex-1 overflow-auto p-4 md:p-6 ${themeClasses.bg}`}>
          {comparisonMode === "side-by-side" && (
            <div className="grid h-full grid-cols-1 gap-4 md:grid-cols-2 md:gap-6">
              <figure
                className={`relative ${themeClasses.bgSecondary} overflow-hidden rounded-xl border shadow-lg ${themeClasses.border}`}
              >
                <img
                ...
                <figcaption className="absolute right-0 bottom-0 left-0 bg-gradient-to-t from-black/70 to-transparent p-4">
                  <span className="text-sm font-semibold text-white md:text-base">{original.label}</span>
                </figcaption>
              </figure>
              <figure
                className={`relative ${themeClasses.bgSecondary} overflow-hidden rounded-xl border shadow-lg ${themeClasses.border}`}
              >
                <img
                  src={`data:image/png;base64,${processed.data}`}
                  alt={processed.label}
                  className="h-full w-full object-contain"
                />
                <figcaption className="absolute right-0 bottom-0 left-0 bg-gradient-to-t from-black/70 to-transparent p-4">
                  <span className="text-sm font-semibold text-white md:text-base">{processed.label}</span>
                </figcaption>
              </figure>
            </div>
          )}
            ...
            <div className="flex h-full flex-col gap-4">
              ...
              >
                ...
                <img
                  src={`data:image/png;base64,${processed.data}`}
                  alt={processed.label}
                  className="overlay-top-img absolute inset-0 h-full w-full object-contain"
                  style={{ opacity: 0.5 }}
                />
              </div>
              ...
              <input
                ...
                defaultValue={50}
                onChange={(e) => {
                  const img = document.querySelector(".overlay-top-img") as HTMLElement | null;
                  if (img) {
                    img.style.opacity = (Number(e.target.value) / 100).toString();
                  }
                }}
